...
const SumOfSquares = ({ num1, num2 }) => (
  <p className="sum-result">{num1 ** 2 + num2 ** 2}</p>
);
...
class TemperatureConverter extends Component {
  ...
  handleChange = (e) => {
    this.setState({ temp: e.target.value, scale: e.target.name });
  };
...
const LAB13 = () => {
  return (
    <div className="container">
      ...
      <UserGreeting firstName="Anushka" lastName="Rai" />
    </div>
  );
};
